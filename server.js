...
// This shortens the Snap page's own reopen window from Midtrans's default
// of 24 hours down to something sane, so a stale checkout link can't be
// confusingly reopened/reused much later. Note: per Midtrans's docs, page
// expiry does NOT send a webhook and is not by itself what unblocks a
// re-booking attempt — that's PENDING_PAYMENT_MAX_AGE_MS below, which is
// our own app-level backstop and the actual fix for the bug described next
// to it. This constant is a complementary UX/hygiene improvement.
const SNAP_PAGE_EXPIRY_MINUTES = 60;
...
app.post('/api/auth/forgot-password', authLimiter, async(req,res)=>{
  ...
  try{
    ...
    const computedExpiresAt = new Date(Date.now() + 60*60*1000).toISOString();
    const {data:savedRow,error:updateErr} = await supabase
      .from('members')
      .update({
      ...
      .eq('id', member.id)
      .select('reset_token_expires').maybeSingle();
    if(updateErr){
...
app.post('/api/auth/reset-password', authLimiter, async(req,res)=>{
  ...
  try{
    ...
    if(!member.reset_token_expires || Number.isNaN(expiresAtMs)){
      console.error('reset-password expiry check: unparseable reset_token_expires for member', member.id, '-> raw value:', JSON.stringify(member.reset_token_expires));
      return res.status(400).json({error:'This password reset link is invalid. Please request a new one.'});
    }
    if(expiresAtMs < nowMs){
      console.warn('reset-password expiry check: EXPIRED for member', member.id,
        '- reset_token_expires raw:', JSON.stringify(member.reset_token_expires),
        '| expiresAtMs:', expiresAtMs, new Date(expiresAtMs).toISOString(),
        '| serverNowMs:', nowMs, new Date(nowMs).toISOString(),
        '| overdue by ms:', nowMs - expiresAtMs);
      return res.status(400).json({error:'This password reset link has expired (links are valid for 60 minutes). Please request a new one.'});
    }
    if(member.status==='inactive')
...
app.post('/api/payment/create',paymentCreateLimiter,async(req,res)=>{
  ...
  // Catches typos (a missing "@", a cut-off domain) before a slot is ever
  // reserved. Without this, Midtrans rejects the transaction later with an
  // error the guest doesn't understand, so they keep retrying the exact
  // same typo and every attempt burns a schedule slot and a pending row.
  if(!isValidEmailFormat(bookingData.email))
  ...
  try{
    const param={
      ...
      // See SNAP_PAGE_EXPIRY_MINUTES above: without this, an abandoned
      // checkout could block this same person from re-booking this same
      // class for up to 24 hours.
      page_expiry:{duration:SNAP_PAGE_EXPIRY_MINUTES,unit:'minute'}
    };
...
// BUGFIX (critical, this is the actual fix for "can't book"/stuck pending
// reports): this used to be 24 hours. migrations/017_integrity_hardening.sql
// added a unique index that blocks the SAME person (name+email+phone) from
// creating a second pending_bookings/bookings row for the SAME class
// session while an earlier one is still open — good for stopping genuine
// duplicates, but it means whatever row is "still open" keeps blocking
// every retry until something closes it. Nothing about Midtrans's own
// status changes that fast for an abandoned checkout (VA/QRIS payment
// methods commonly stay "pending" in Midtrans's own system for hours by
// default), so this app-level timer — which fires regardless of what
// Midtrans reports — is what actually has to give up soon and release the
// slot + clear the row so the same person can book again. At 24 hours,
// anyone who abandoned a checkout (closed the tab, payment glitched,
// changed their mind) was locked out of re-booking that class for up to a
// full day. 90 minutes still comfortably covers a real, slow, legitimate
// payment while no longer leaving people locked out for most of a day.
const PENDING_PAYMENT_MAX_AGE_MS = 90*60*1000;
// Midtrans returning an actual "still pending" status (an unpaid bank-transfer
// VA, say) gets that payment method's own natural window above. A `null`
// result means the status check itself failed — overwhelmingly because
// Midtrans has never heard of this order at all, i.e. the guest closed the
// tab before the payment widget ever loaded. That is, if anything, a
// STRONGER sign this was abandoned than a genuine "still pending", so it
// gets a much shorter leash instead of the two silently sharing one
// long threshold that only one of them can ever actually reach — which is
// exactly what let real, unrecoverable pending bookings pile up for weeks
// with the reconcile sweep running the whole time and never touching them.
const PENDING_PAYMENT_UNKNOWN_MAX_AGE_MS = 30*60*1000;
...
async function expireUnpaidPendingIfAged(orderId,isPackage,createdAt,result){
  if(result?.confirmed || result?.cancelled || !createdAt) return result;
  const age=Date.now()-new Date(createdAt).getTime();
  if(!Number.isFinite(age)) return result;
  const threshold = result ? PENDING_PAYMENT_MAX_AGE_MS : PENDING_PAYMENT_UNKNOWN_MAX_AGE_MS;
  if(age<threshold) return result;
...
app.post('/api/membership-purchase/create',requireMember,async(req,res)=>{
  ...
  if(!isValidPhoneFormat(memberPhone))
    return res.status(400).json({error:'Please enter a valid WhatsApp number.'});
...
<p style="font-size:14px;color:#7a6e68;line-height:1.7">We received a request to reset the password for your account. Click the button below to choose a new password. This link expires in 60 minutes.</p>
...
<div class="ftr">This is an automated email from ${studioName}. Please do not reply.</div>
  ...
  const text = `Password Reset — ${studioName}\n\nHi ${member.name||''},\n\nWe received a request to reset your password. Open this link to choose a new one (expires in 60 minutes):\n${resetUrl}\n\nIf you didn't request this, you can ignore this email.`;
  return { subject, html, text };
}
