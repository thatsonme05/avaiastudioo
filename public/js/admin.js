...
async function gateLogin(){
  const u=gv('g-user').trim(),p=gv('g-pass');
  if(!u||!p){showErr('Please enter your username/email and password.');return;}

  const btn=document.querySelector('#gate .btn.bp');
  ...
  try{
    ...
    showErr('Server cannot be reached. Check your website URL/hosting and try again.');
  }finally{
    if(btn){btn.disabled=false;btn.textContent=oldText||'Sign In to Dashboard';}
  }
}
...
async function showAdm(){
  ...
  // Validate the session before rendering the dashboard. This prevents a stale
  // localStorage entry from showing a broken/blank admin panel.
  try{
    ...
    document.getElementById('adm').style.display='none';
    document.getElementById('gate').style.display='flex';
    showErr('Your session expired. Please sign in again.');
    return;
  }
...
function updateSchDayHint(dayName){
  ...
  if(h) h.textContent = dayName ? 'This is a weekly class — it repeats every '+dayName+' at the time below.' : '';
}
function syncSchDayFromDate(){
  const v=gv('sch-date-picker'); if(!v) return;
  const d=new Date(v+'T00:00:00');
  const dayName=SCH_DOW[d.getDay()];
  const sel=document.getElementById('sch-day'); if(sel) sel.value=dayName;
  updateSchDayHint(dayName);
}
function syncSchDateFromDay(){
  const sel=document.getElementById('sch-day'); if(!sel) return;
  sv('sch-date-picker', nearestDateForDayName(sel.value));
  updateSchDayHint(sel.value);
}
function openSchMo(){
  ...
  const daySel=document.getElementById('sch-day'); if(daySel) daySel.selectedIndex=0;
  syncSchDateFromDay();
  openMo('sch-mo');
}
function openEditSch(id){
  ...
  st('sch-mo-t','Edit Session'); sv('sch-eid',s.id); document.getElementById('sch-day').value=s.day; sv('sch-time',s.time); sv('sch-slots',s.slots);
  syncSchDateFromDay();
  openMo('sch-mo');
}
async function saveSch(){
  const eid=gv('sch-eid'), body={day:document.getElementById('sch-day').value,time:gv('sch-time'),class_id:document.getElementById('sch-cls').value,slots:parseInt(gv('sch-slots'))||12};
...
async function loadSysStatus(){
  ...
  try{
    ...
    if(ss) ss.innerHTML='<div>Database: <strong>'+(d.supabase?'Supabase ☁':'db.json (local)')+'</strong></div>'+
...
document.addEventListener('DOMContentLoaded',()=>{
  loadCurrentStaff();
  if(CURRENT_STAFF && localStorage.getItem('avaia_staff')) showAdm();
