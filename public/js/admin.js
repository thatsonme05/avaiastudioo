...
function updateStats(){
  ...
  loadDashboardRevenue(); // fetches the accurate total (bookings + membership packages) from the server
...
function populateDbRevYearFilter(){
  const sel=document.getElementById('db-rev-f-year'); if(!sel) return;
...
function onDbRevDateFilterChange(){
  if(gv('db-rev-f-date')){ sv('db-rev-f-month',''); sv('db-rev-f-year',''); }
  applyDbRevenueFilter();
}
function applyDbRevenueFilter(){
  const date=gv('db-rev-f-date');
  if(date){ sv('db-rev-f-month',''); sv('db-rev-f-year',''); }
  const month=gv('db-rev-f-month');
  const year=gv('db-rev-f-year');
  loadDashboardRevenue({date,month,year});
}
function clearDbRevenueFilter(){
  sv('db-rev-f-date',''); sv('db-rev-f-month',''); sv('db-rev-f-year','');
  loadDashboardRevenue();
}
async function loadDashboardRevenue(filter){
  if(!isAdminRole()) return;
  try{
    populateDbRevYearFilter();
    let url='/api/stats/admin';
    if(filter && (filter.date||filter.month||filter.year)){
      const qs=new URLSearchParams();
      if(filter.date){ qs.set('from',filter.date); qs.set('to',filter.date); }
    ...
    if(d.error) return;
    const usingFilter = d.summary.filteredRange!=null;
    st('s-rev', fmtRp(usingFilter ? d.summary.filteredRevenue : d.summary.totalRevenue));
  }catch(e){ /* leave last known value on transient failure */ }
}
...
async function loadABk(){
  ...
  populateBkYearFilter();
  renderABkTable();
}
// Booking dates are normally the app's own "September 1, 2026" style string,
// but bookings imported from Excel keep whatever text was in that column, so
// parsing can fail for oddly-formatted imports — returns null rather than an
// invalid Date so callers can just skip those rows from a narrowed filter.
function parseBkDate(dateStr){
  if(!dateStr) return null;
  const d=new Date(dateStr);
  return isNaN(d.getTime()) ? null : d;
}
function populateBkYearFilter(){
  const sel=document.getElementById('bk-f-year'); if(!sel) return;
  const current=sel.value;
  const years=[...new Set(ABk.map(b=>{const d=parseBkDate(b.date);return d?d.getFullYear():null;}).filter(Boolean))].sort((a,b)=>b-a);
  sel.innerHTML='<option value="">All Years</option>'+years.map(y=>'<option value="'+y+'">'+y+'</option>').join('');
  if(years.includes(Number(current))) sel.value=current;
}
function onBkDateFilterChange(){
  // An exact date is the most specific filter, so picking one clears the
  // broader month/year dropdowns rather than trying to combine with them.
  if(gv('bk-f-date')){ sv('bk-f-month',''); sv('bk-f-year',''); }
  renderABkTable();
}
function clearBkFilters(){
  sv('bk-f-name',''); sv('bk-f-date',''); sv('bk-f-month',''); sv('bk-f-year','');
  renderABkTable();
}
function renderABkTable(){
  const ab=document.getElementById('all-bk-body'); if(!ab) return;
  const fName=gv('bk-f-name').trim().toLowerCase(); // free-text substring match on name
  const fDate=gv('bk-f-date');   // yyyy-mm-dd from <input type="date">, or ''
  const fMonth=gv('bk-f-month'); // '1'-'12', or ''
  const fYear=gv('bk-f-year');   // e.g. '2026', or ''

  const filtered = ABk.filter(b=>{
    if(fName && !String(b.name||'').toLowerCase().includes(fName)) return false;
    if(!fDate && !fMonth && !fYear) return true;
    const d=parseBkDate(b.date);
    if(!d) return false;
    if(fDate){
  ...
  const countEl=document.getElementById('bk-f-count');
  if(countEl) countEl.textContent = (fName||fDate||fMonth||fYear) ? ('Showing '+filtered.length+' of '+ABk.length+' bookings') : '';

  const _tok=localStorage.getItem('avaia_staff')||'';
  ab.innerHTML=filtered.map(b=>{
  ...
  }).join('')||'<tr><td colspan="9" style="padding:16px;color:var(--mt)">'+(ABk.length?'No bookings match this filter.':'None yet.')+'</td></tr>';
}
...
async function reconcilePending(){
  ...
  if(!confirm('This checks every still-pending booking and membership payment directly with Midtrans, and finalizes any that were actually paid. Genuinely unpaid ones are left alone (or marked cancelled if Midtrans reports them expired/denied). Continue?')) return;
  btn.disabled=true; btn.textContent='Checking…';
  box.style.display='block'; box.innerHTML='Checking pending payments against Midtrans — this can take a moment if there are many…';
  try{
    const r=await fetch('/api/admin/reconcile-pending',{method:'POST'});
    const d=await r.json();
    if(!d.ok){ box.innerHTML='<strong style="color:var(--bad,#a33d3d)">Failed:</strong> '+(d.error||'Unknown error.'); return; }
    if(!d.scanned){ box.innerHTML='No pending payments found — everything is already settled.'; }
    else{
      const lines=d.results.map(x=>{
        const label = x.outcome==='confirmed' ? '<span style="color:var(--ok,#4a7a48)">✓ confirmed</span>'
          : x.outcome==='cancelled' ? '<span style="color:var(--mt)">cancelled (unpaid/expired)</span>'
          : x.outcome==='check_failed' ? '<span style="color:var(--bad,#a33d3d)">check failed — '+(x.error||'')+'</span>'
          : '<span style="color:var(--mt)">still pending</span>';
        return '<div>'+ (x.type==='membership'?'Membership':'Booking') +' — '+(x.who||'—')+' ('+(x.what||'—')+'): '+label+'</div>';
      }).join('');
      box.innerHTML = '<div style="margin-bottom:10px"><strong>Scanned '+d.scanned+'</strong> — '
        + d.confirmed+' confirmed, '+d.cancelled+' cancelled, '+d.stillPending+' still pending'
        + (d.failed?', '+d.failed+' failed to check':'') + '.</div>' + lines;
      if(d.confirmed){ await Promise.all([loadABk(),loadAMb()]); updateStats(); }
    }
  }catch(e){
    box.innerHTML='<strong style="color:var(--bad,#a33d3d)">Failed:</strong> '+e.message;
  }finally{
    btn.disabled=false; btn.textContent='Check Pending Payments';
  }
}
...
/* ── MEMBERS (admin only) ── */
let AMPkg=[];
function renderMemberPkgLine(p){
  const left=Math.max(0,p.credits_total-p.credits_used);
  const exp=new Date(p.expires_at).toLocaleDateString('en-US',{day:'numeric',month:'short'});
  const statusLabel=p.computed_status==='active'?'':` <span style="color:${p.computed_status==='depleted'?'#b52b2b':'var(--mt)'}">(${p.computed_status})</span>`;
  return '<div style="margin-bottom:4px">'+p.package_name+statusLabel+' <span style="color:var(--mt)">('+left+' of '+p.credits_total+' left, exp: '+exp+')</span></div>';
}
...
function renderAMbTable(){
  ...
  const fName=gv('mb-f-name').trim().toLowerCase();
  const rows=fName?AMb.filter(m=>String(m.name||'').toLowerCase().includes(fName)):AMb;

  const countEl=document.getElementById('mb-f-count');
  if(countEl) countEl.textContent = fName ? ('Showing '+rows.length+' of '+AMb.length+' members') : '';

  b.innerHTML=rows.length?rows.map(m=>{
    ...
    const pkgs=AMPkg.filter(p=>(p.member_id&&p.member_id===m.id)||String(p.member_email||'').trim().toLowerCase()===memberEmail);
    // BUGFIX (structural, not just a heuristic tweak): this table used to
    // pick and show only ONE package per member — whichever expired
    // soonest. A member can genuinely have more than one package active at
    // the same time (e.g. an older "5 Classes" pack they haven't finished
    // yet, plus a brand-new "10 Classes" pack bought before the first ran
    // out). With only one shown, the OTHER package became completely
    // invisible and uneditable from this table — its credits could never be
    // corrected here, no matter how many times the visible one was edited,
    // because they are different database rows. That's what caused a
    // member whose membership_type said "10 Classes" to only ever show/edit
    // a "5 Classes" package that kept reappearing after being changed: the
    // real 10-class row was simply never being touched.
    // Fix: list EVERY active or depleted package for this member (soonest
    // expiry first, actives before depleted). Package edits are deliberately
    // kept behind the row's single Edit button below so credit changes are
    // not accidental inline actions in this read-only summary column.
    const activeOnes=pkgs.filter(p=>p.computed_status==='active').sort((a,b)=>new Date(a.expires_at)-new Date(b.expires_at));
    const depletedOnes=pkgs.filter(p=>p.computed_status==='depleted').sort((a,b)=>new Date(a.expires_at)-new Date(b.expires_at));
    const visiblePkgs=[...activeOnes,...depletedOnes];
    const pkgCell=visiblePkgs.length?visiblePkgs.map(renderMemberPkgLine).join(''):'<span style="color:var(--mt)">— drop-in —</span>';
    // The "Membership Type" badge lists every currently-active package name
    // (not just one), so it can never show something the row below it
    // doesn't also show and let you edit.
    const membershipTypeLabel=activeOnes.length?activeOnes.map(p=>p.package_name).join(' + '):'drop-in';
    return '<tr><td>'+m.name+'</td><td>'+m.email+'</td><td>'+(m.phone||'—')+'</td><td style="text-transform:capitalize">'+membershipTypeLabel+'</td><td style="font-size:12.5px">'+pkgCell+'</td><td>'+j+'</td><td><div style="display:flex;gap:6px"><button class="btn bo bsm" onclick="openEditMb(\''+m.id+'\')">Edit</button><button class="btn bd bsm" onclick="delMb(\''+m.id+'\')">Delete</button></div></td></tr>';
  }).join(''):'<tr><td colspan="7" style="padding:16px;color:var(--mt)">'+(AMb.length?'No members match this search.':'No members yet.')+'</td></tr>';
}

/* ── EDIT MEMBER + ITS MEMBERSHIP (all changes live behind one Edit button) ── */
function openEditMb(id){
  const m=AMb.find(x=>x.id===id); if(!m) return;
  sv('emb-eid',m.id); sv('emb-name',m.name); sv('emb-email',m.email);
  sv('emb-phone',m.phone||''); sv('emb-mtype',m.membership_type||'drop-in');
  const select=document.getElementById('emb-pkg');
  const memberEmail=String(m.email||'').trim().toLowerCase();
  const pkgs=AMPkg.filter(p=>(p.member_id&&p.member_id===m.id)||String(p.member_email||'').trim().toLowerCase()===memberEmail)
    .sort((a,b)=>({active:0,depleted:1,expired:2,cancelled:3}[a.computed_status]??4)-({active:0,depleted:1,expired:2,cancelled:3}[b.computed_status]??4)||new Date(a.expires_at)-new Date(b.expires_at));
  if(select){
    select.innerHTML='<option value="">Do not edit a package</option>'+pkgs.map(p=>`<option value="${p.id}">${p.package_name} — ${p.credits_total-p.credits_used} left (${p.computed_status})</option>`).join('');
    select.value=pkgs[0]?.id||'';
  }
  loadEditMemberPackage();
  openMo('emb-mo');
}

function loadEditMemberPackage(){
  const id=gv('emb-pkg');
  const p=AMPkg.find(x=>x.id===id);
  const fields=document.getElementById('emb-pkg-fields');
  if(!p){ if(fields) fields.style.display='none'; return; }
  if(fields) fields.style.display='block';
  sv('emb-pkg-id',p.id); sv('emb-pkg-name',p.package_name||''); sv('emb-pkg-price',p.price_paid??0);
  sv('emb-pkg-total',p.credits_total??1); sv('emb-pkg-used',p.credits_used??0);
  sv('emb-pkg-expires',p.expires_at?new Date(p.expires_at).toISOString().slice(0,10):'');
  const pay=document.getElementById('emb-pkg-payment');
  if(pay) pay.value=['manual','cash','bank_transfer','qris_offline','online'].includes(p.payment_type)?p.payment_type:'manual';
  const status=document.getElementById('emb-pkg-status');
  if(status) status.value=['active','cancelled','expired'].includes(p.status)?p.status:'active';
}

async function saveEditMb(){
  const id=gv('emb-eid');
  const memberBody={name:gv('emb-name'),email:gv('emb-email'),phone:gv('emb-phone'),membership_type:gv('emb-mtype')};
  if(!memberBody.name||!memberBody.email){ toast('Name and email are required.','err'); return; }
  const packageId=gv('emb-pkg');
  let packageBody=null;
  if(packageId){
    const total=Number(gv('emb-pkg-total')), used=Number(gv('emb-pkg-used')), price=Number(gv('emb-pkg-price'));
    if(!gv('emb-pkg-name')||!gv('emb-pkg-expires')){toast('Package name and expiry date are required.','err');return;}
    if(!Number.isInteger(total)||total<1||!Number.isInteger(used)||used<0||used>total||!Number.isInteger(price)||price<0){toast('Enter valid package credits and price values.','err');return;}
    packageBody={package_name:gv('emb-pkg-name'),price_paid:price,credits_total:total,credits_used:used,
      expires_at:gv('emb-pkg-expires')+'T23:59:59',payment_type:document.getElementById('emb-pkg-payment')?.value||'manual',
      status:document.getElementById('emb-pkg-status')?.value||'active'};
  }
  const btn=document.getElementById('emb-save-btn');
  if(btn){if(btn.disabled)return;btn.disabled=true;btn.textContent='Saving…';}
  try{
    const r=await fetch('/api/members/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(memberBody)});
    if(!(await checkRes(r))) return;
    if(packageBody){
      const rp=await fetch('/api/member-packages/'+packageId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(packageBody)});
      if(!(await checkRes(rp))){await loadAMb();return;}
    }
    closeMo('emb-mo'); await loadAMb(); updateStats(); toast(packageBody?'Member and membership updated.':'Member updated.','ok');
  }catch(e){toast('Could not save the changes.','err');}
  finally{if(btn){btn.disabled=false;btn.textContent='Save Changes';}}
}

async function deleteEditedMemberPackage(){
  const id=gv('emb-pkg'); if(!id) return;
  if(!confirm('Remove this membership entirely? This cannot be undone.')) return;
  const r=await fetch('/api/member-packages/'+id,{method:'DELETE'});
  if(!(await checkRes(r))) return;
  closeMo('emb-mo'); await loadAMb(); updateStats(); toast('Membership removed.');
}
...
async function loadACls(){
  ...
  b.innerHTML=ACls.map(c=>{
    const priceLabel = (c.price && c.price>0) ? fmtRp(c.price) : '<span style="color:#a33d3d">⚠ Not set</span>';
    return '<tr><td>'+c.name+'</td><td>'+c.instructor+'</td><td>'+c.level+'</td><td>'+c.duration+'</td><td>'+priceLabel+'</td><td>'+c.capacity+'</td><td><div style="display:flex;gap:6px"><button class="btn bo bsm" onclick="openEditCls(\''+c.id+'\')">Edit</button><button class="btn bd bsm" onclick="delCls(\''+c.id+'\')">Delete</button></div></td></tr>';
  }).join('');
}
...
function openEditCls(id){
  const c=ACls.find(x=>x.id===id); if(!c) return;
  st('cls-mo-t','Edit Class'); sv('cls-eid',c.id);sv('cls-name',c.name);sv('cls-inst',c.instructor);
  document.getElementById('cls-lv').value=c.level;sv('cls-dur',c.duration);sv('cls-cap',c.capacity);
  // Leave the price field blank rather than silently pre-filling 85000 when
  // no price is set — pre-filling made it look like a real saved value, so
  // an admin who didn't touch the field would unintentionally lock the
  // class's price at 85000 on save. This is what caused bookings to keep
  // charging Rp85,000 even after the studio set the real price elsewhere.
  sv('cls-price', c.price && c.price>0 ? c.price : '');
  openMo('cls-mo');
}
async function saveCls(){
  const id=gv('cls-eid');
  const priceRaw=gv('cls-price');
  const price=parseInt(priceRaw,10);
  if(!priceRaw || !Number.isFinite(price) || price<=0){
    toast('Please enter a valid price greater than 0 — it will not default to any placeholder value.','err');
    return;
  }
  const body={name:gv('cls-name'),instructor:gv('cls-inst'),level:document.getElementById('cls-lv').value,duration:gv('cls-dur'),capacity:parseInt(gv('cls-cap'))||12,price};
...
async function saveManualPkg(){
  ...
  // BUGFIX: this is the actual cause of members ending up with a dozen-plus
  // duplicate manual packages (found in production data — one member had 13
  // identical "5 Classes" packages all added on the same day). Nothing
  // stopped this button from being clicked — or the "Activate Membership"
  // click registering twice on a slow connection — more than once before
  // the modal closed, and every click fired its own independent, full
  // insert with no idempotency check on the server side (unlike online
  // payments, a manual sale has no orderId to de-duplicate by). Disabling
  // the button for the duration of the request, and bailing out immediately
  // if a save is already in flight, closes this off at the source.
  ...
  finally{ if(btn){ btn.disabled=false; btn.textContent='Activate Membership'; } }
}
...
async function saveMsh(){
  const id=gv('msh-eid');
  const price=Number(gv('msh-price')),credits=Number(gv('msh-credits')),validity=Number(gv('msh-validity'));
  if(!gv('msh-name')||!Number.isInteger(price)||price<=0||!Number.isInteger(credits)||credits<=0||!Number.isInteger(validity)||validity<=0){
    toast('Package name, price, credits and validity must all be valid positive values.','err'); return;
  }
  const body={name:gv('msh-name'),price,duration:gv('msh-duration'),
    credits, validity_days:validity,
...
let _anLoaded=false;
let _anData=null;
let _anPeriod='monthly';
function setAnalyticsPeriod(period){
  _anPeriod=period;
  const mBtn=document.getElementById('an-period-monthly'), wBtn=document.getElementById('an-period-weekly');
  if(mBtn) mBtn.classList.toggle('active',period==='monthly');
  if(wBtn) wBtn.classList.toggle('active',period==='weekly');
  renderRevenueChart();
}
function renderRevenueChart(){
  if(!_anData) return;
  const d=_anData;
  const isWeekly=_anPeriod==='weekly';
  const series=isWeekly?d.weekly12:d.monthly12;
  const ttl=document.getElementById('an-rev-chart-ttl');
  if(ttl) ttl.textContent=isWeekly?'Revenue — Last 12 Weeks':'Revenue — Last 12 Months';

  const growEl=document.getElementById('an-rev-grow');
  if(growEl){
    if(d.summary.filteredRange!=null){
      growEl.className='stat-grow';
      growEl.textContent=d.summary.filteredBookingsCount+' paid booking'+(d.summary.filteredBookingsCount===1?'':'s');
    } else {
      const g=isWeekly?d.summary.weeklyRevenueGrowthPct:d.summary.revenueGrowthPct;
      growEl.className='stat-grow '+(g>=0?'up':'down');
      growEl.textContent=(g>=0?'▲ ':'▼ ')+Math.abs(g)+'% vs last '+(isWeekly?'week':'month');
    }
  }

  const maxRev=Math.max(1,...series.map(m=>m.revenue));
  const mc=document.getElementById('an-monthly-chart');
  if(mc) mc.innerHTML=series.map(m=>`
    <div class="bchart-col" title="${m.label}: ${fmtRp(m.revenue)} (${m.bookings} paid booking${m.bookings===1?'':'s'})">
      <div class="bchart-bar" style="height:${Math.max(2,(m.revenue/maxRev)*100)}%"></div>
      <div class="bchart-label">${m.label}</div>
    </div>`).join('');
}
function populateAnRevYearFilter(){
  const sel=document.getElementById('an-rev-f-year'); if(!sel) return;
  const current=sel.value;
  const curY=new Date().getFullYear();
  const years=[]; for(let y=curY;y>=curY-5;y--) years.push(y);
  sel.innerHTML='<option value="">All Years</option>'+years.map(y=>`<option value="${y}">${y}</option>`).join('');
...
function onAnRevDateFilterChange(){
  // An exact date is the most specific filter, so picking one clears the
  // broader month/year dropdowns rather than trying to combine with them
  // (same rule as the Bookings tab's date filter).
  if(gv('an-rev-f-date')){ sv('an-rev-f-month',''); sv('an-rev-f-year',''); }
  applyRevenueFilter();
}
function applyRevenueFilter(){
  const date=gv('an-rev-f-date');
  if(date){ sv('an-rev-f-month',''); sv('an-rev-f-year',''); }
  const month=gv('an-rev-f-month');
  const year=gv('an-rev-f-year');
  loadAnalytics({date,month,year});
}
function clearRevenueFilter(){
  sv('an-rev-f-date',''); sv('an-rev-f-month',''); sv('an-rev-f-year','');
  loadAnalytics();
}
async function loadAnalytics(filter){
  if(!isAdminRole()) return;
  try{
    populateAnRevYearFilter();
    let url='/api/stats/admin';
    if(filter && (filter.date||filter.month||filter.year)){
      const qs=new URLSearchParams();
      if(filter.date){ qs.set('from',filter.date); qs.set('to',filter.date); }
    ...
    const usingFilter = d.summary.filteredRange!=null;
    st('an-rev', fmtRp(usingFilter ? d.summary.filteredRevenue : d.summary.totalRevenue));
    ...
    // Revenue chart (Monthly/Weekly toggle)
    renderRevenueChart();
