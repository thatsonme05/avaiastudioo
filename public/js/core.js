...
window.fetch = function(url, options = {}) {
  ...
  return _origFetch(url, options).then(res => {
    if (res.status === 401 && url.includes('/api/') && !url.includes('/api/auth/') && !url.includes('/api/notifications')) {
      ...
      // (Background notification polling is excluded — a single flaky check there
      // shouldn't silently sign someone out while they're actively using the site.)
...
function showBootError(step, e){
  console.error(step+' failed:', e);
  let box = document.getElementById('_boot_err');
  if (!box) {
    box = document.createElement('div');
    box.id = '_boot_err';
    box.style.cssText = 'position:fixed;bottom:12px;left:12px;right:12px;z-index:99999;background:#b52b2b;color:#fff;padding:12px 16px;border-radius:8px;font:12px/1.5 monospace;max-height:40vh;overflow:auto;box-shadow:0 4px 20px rgba(0,0,0,.3)';
    document.body.appendChild(box);
  }
  const line = document.createElement('div');
  line.style.marginBottom = '4px';
  line.textContent = '[' + step + '] ' + (e && e.message ? e.message : String(e));
  box.appendChild(line);
}

/* ── BOOT ── */
async function bootApp(){
  try{ injectNav(); }catch(e){ showBootError('injectNav', e); }
  try{ injectModals(); }catch(e){ showBootError('injectModals', e); }
  try{ applyI18n(); }catch(e){ showBootError('applyI18n', e); }

  const settled = await Promise.allSettled([loadST(), loadPayStatus()]);
  settled.forEach(r=>{ if(r.status==='rejected') showBootError('loadST/loadPayStatus', r.reason); });

  try{ restoreUser(); }catch(e){ showBootError('restoreUser', e); }
  try{ renderNav(); }catch(e){ showBootError('renderNav', e); }
  try{ renderFooter(); }catch(e){ showBootError('renderFooter', e); }
  try{ initSocket(); }catch(e){ showBootError('initSocket', e); }
  if(CU){ try{ loadMemberNotifs(); }catch(e){ showBootError('loadMemberNotifs', e); } }
  if(typeof pageInit==='function'){
    try{ await pageInit(); }catch(e){ showBootError('pageInit', e); }
  }
  try{ applyI18n(); }catch(e){ showBootError('applyI18n#2', e); }
}
document.addEventListener('DOMContentLoaded',bootApp);

// Safari (and other browsers) can restore a page from the back-forward
// cache when navigating Back/Forward, showing a frozen snapshot from
// before login state or async content (like homepage photos) finished
// loading, instead of running the page fresh. Re-boot when that happens.
window.addEventListener('pageshow',(e)=>{
  if(e.persisted) bootApp();
});

/* ── SETTINGS ── */
async function loadST(){
  try{ const r=await fetch(API+'/api/settings'); ST=await r.json(); applyTheme(); }
...
function restoreUser(){
  ...
  if(!s) return;
  try{ CU=JSON.parse(s); }
  catch(e){ console.error('Corrupted session data, clearing:', e); localStorage.removeItem('avaia_u'); localStorage.removeItem('avaia_member_token'); CU=null; }
}
function saveUser(u){ CU=u; localStorage.setItem('avaia_u',JSON.stringify(u)); }
function clearUser(){ CU=null; localStorage.removeItem('avaia_u'); localStorage.removeItem('avaia_member_token'); }
...
async function doLogin(){
  ...
  try{
    // Try as member first (using email)
    const r1=await fetch(API+'/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:identity,password:pass})});
    const d1=await r1.json();
    if(d1.ok){
      localStorage.setItem('avaia_member_token',d1.token);
      saveUser(d1.user);closeMo('login-mo');renderNav();
      toast(t('msg.welcomeback')+' '+(d1.user.name||d1.user.email).split(' ')[0]+'!','ok');
      loadMemberNotifs();
      if(typeof onAfterLogin==='function') onAfterLogin(d1.user);
      return;
    }

    // If that fails, try as staff (admin/instructor)
    const r2=await fetch(API+'/api/auth/staff-login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:identity,password:pass})});
    const d2=await r2.json();
    if(d2.ok){
      localStorage.setItem('avaia_staff',d2.token);
      localStorage.setItem('avaia_staff_user',JSON.stringify(d2.user));
      closeMo('login-mo');
      toast(t('msg.welcomeback')+' '+d2.user.name+'!','ok');
      setTimeout(()=>window.location.href='/admin',500);
      return;
    }

    toast(t('msg.wrongcreds'),'err');
...
function doLogout(){clearUser();closeDrawer();renderNav();toast(t('msg.signedout'));setTimeout(()=>window.location.href='/',600);}
...
function injectModals(){
  ...
  <div class="mo" id="login-mo">
    <div class="mbox" style="max-width:420px">
      ...
      <div class="fg"><label class="fl">${t('modal.login.email')}</label><input type="text" class="fi" id="li-email" placeholder="email@example.com" onkeydown="if(event.key==='Enter')doLogin()"></div>
...
// Returns the next `count` upcoming calendar dates that fall on `dayName`
// (e.g. next 8 Mondays), formatted to match the "D Month YYYY" style used
// across the app (and expected by the server's cancellation-deadline parser).
function upcomingDatesForDay(dayName,count=8){
  const di=DAY.indexOf(dayName);
  const now=new Date();
  let diff=di-now.getDay(); if(diff<0) diff+=7;
  const out=[];
  for(let i=0;i<count;i++){
    const t=new Date(now); t.setDate(now.getDate()+diff+7*i);
    out.push(t.getDate()+' '+MON[t.getMonth()]+' '+t.getFullYear());
  }
  return out;
}

function openBooking(schedId,name,dayName,preferredDateStr,time,instructor,slots,price){
  const dates=upcomingDatesForDay(dayName,8);
  let defaultIdx=dates.indexOf(preferredDateStr);
  if(defaultIdx<0) defaultIdx=0;
  _BK={schedId,name,dayName,dateStr:dates[defaultIdx],time,instructor,slots,price:price||85000};
  ...
  if(dsel){
    dsel.innerHTML=dates.map((d,i)=>`<option value="${i}">${d}</option>`).join('');
    dsel.value=String(defaultIdx);
  }
...
function onBookingDateChange(){
  ...
  const dsel=document.getElementById('bk-date-select');
  if(!dsel) return;
  const dates=upcomingDatesForDay(_BK.dayName,8);
  _BK.dateStr=dates[parseInt(dsel.value,10)]||dates[0];
}
