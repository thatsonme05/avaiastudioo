...
function openBooking(schedId,name,dayName,preferredDateStr,time,instructor,slots,price){
  ...
  // Never turn a missing/invalid price into IDR 0. The server verifies the
  // class price from the database; this guard only prevents a stale page from
  // showing a misleading zero before that request is made.
  const numericPrice=Number(price);
  const hasValidPrice=Number.isFinite(numericPrice)&&numericPrice>0;
  _BK={schedId,name,dayName,dateStr,time,instructor,slots,price:hasValidPrice?numericPrice:null};
  ...
  qs('#bk-price').textContent=hasValidPrice?'IDR '+numericPrice.toLocaleString('en-US'):'Unavailable';
  if(CU){
  ...
  if(btn){btn.textContent='Continue to Payment';btn.disabled=!hasValidPrice;}
  const info=document.getElementById('bk-pay-info');
  if(info) info.innerHTML=!hasValidPrice
    ?'<span style="color:var(--a)">The price for this class is not available yet. Please contact the studio.</span>'
    :_payStatus.configured
...
async function ensureSnapLoaded(){
  ...
  if(!_payStatus.clientKey) throw new Error('Payment gateway is not configured.');
  ...
  await new Promise((resolve,reject)=>{
    ...
    timer=setTimeout(()=>finish(reject,new Error('Payment gateway timed out while loading.')),12000);
    s.onload=()=>window.snap
      ? finish(resolve)
      : finish(reject,new Error('Payment gateway loaded incorrectly.'));
    s.onerror=()=>finish(reject,new Error('Payment gateway could not be loaded.'));
  });
}
...
async function submitBookingWithPayment(){
  if(!_BK || !Number.isFinite(_BK.price) || _BK.price<=0){
    toast('The price for this class is not available yet. Please contact the studio.','err');
    return;
  }
  ...
  try{
    const r=await fetchWithTimeout(API+'/api/payment/create',{method:'POST',headers:{'Content-Type':'application/json'},
    ...
    if(txn.redeemed){
      ...
      const r2=await fetchWithTimeout(API+'/api/payment/simulate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({orderId:txn.orderId})});
...
function openPackagePurchase(pkg){
  ...
  if(!Number.isFinite(numericPrice)||numericPrice<=0){
    toast('This membership package is not available for purchase yet. Please contact the studio.','err');
    return;
  }
  _PKG={...pkg,price:numericPrice};
  ...
  qs('#pkg-price').textContent='IDR '+numericPrice.toLocaleString('en-US');
...
async function submitPackagePurchase(){
  ...
  if(btn){btn.textContent=t('msg.processing');btn.disabled=true;}
  try{
    const r=await fetchWithTimeout(API+'/api/membership-purchase/create',{method:'POST',headers:{'Content-Type':'application/json'},
    ...
    if(txn.simulated){
      const r2=await fetchWithTimeout(API+'/api/membership-purchase/simulate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({orderId:txn.orderId})});
      const sim=await r2.json();
      if(sim.ok){
