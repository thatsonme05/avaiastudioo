...
async function doLogin(){
  ...
  try{
    // Staff usernames are routed directly to the staff endpoint.
    // This is intentional: a value such as "admin" is not an email address
    // and must never be sent through the member login flow.
    const isStaffIdentity = !identity.includes('@');
    const loginEndpoint = isStaffIdentity ? '/api/auth/staff-login' : '/api/auth/login';
    const loginBody = isStaffIdentity
      ? {username: identity, password: pass}
...
function injectModals(){
  ...
  <div class="mo" id="login-mo">
    <div class="mbox" style="max-width:420px">
      ...
      <div class="fg"><label class="fl">Email or Username</label><input type="text" class="fi" id="li-email" placeholder="email@example.com or admin" onkeydown="if(event.key==='Enter')doLogin()"></div>
